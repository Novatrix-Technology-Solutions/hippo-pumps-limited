            ...
            <div className="container mx-auto px-4 py-8">
                ...
                {news.data.length === 0 ? (
                    <div className="text-center py-12">
                        <p className="text-gray-600 text-lg">There are no news articles available at this time. Please check back later.</p>
                    </div>
                ) : (
                    <div className="grid gap-8 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                        {news.data.map((item) => (
                            <Link key={item.id} href={route('public.news.show', item.slug)}>
                                <article className="bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-shadow h-full">
                                    {item.featured_image && (
                                        <img
